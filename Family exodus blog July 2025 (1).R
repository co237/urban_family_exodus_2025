...
# Load in Data #
################
# county population data https://www.census.gov/data/tables/time-series/demo/popest/2020s-counties-detail.html
...
## GRAPH: Annual change in age 0-4 population by county type ##
...
# overall for NYC counties
nyc_under_5_total <- exodus_data %>%
  select(FIPS, county_nm, Age_group, YEAR, TOT_POP) %>%
  filter(county_nm == "Kings County, New York" |
           county_nm == "Queens County, New York" |
           county_nm == "New York County, New York" |
  ...
  mutate(pc_21 = (July_2021-July_2020)/July_2020*100,
         ...
         pc_24 = (July_2024-July_2023)/July_2023*100,
         pc_since_20 = (July_2024-April_2020)/April_2020*100
  )  %>% mutate(county_nm = "New York City (Overall)") %>%
    select(county_nm, pc_21, pc_22, pc_23, pc_24)


## GRAPH: Change in age 0-4 population, New York City
...
## Deline in selected comparable metros 
under_5_panel_large_urban %>%
  filter(county_nm == "San Francisco County, California") %>% select(contains("pc"))

under_5_panel_large_urban %>%
  filter(county_nm == "Los Angeles County, California") %>% select(contains("pc"))

under_5_panel_large_urban %>%
  filter(county_nm == "Orleans Parish, Louisiana") %>% select(contains("pc"))

under_5_panel_large_urban %>%
  filter(county_nm == "Philadelphia County, Pennsylvania") %>% select(contains("pc"))

under_5_panel_large_urban %>%
  filter(county_nm == "Suffolk County, Massachusetts") %>% select(contains("pc"))



## TABLE: Large urban counties with fastest-shrinking age 0-4 populations since 2020 ##
...
## TABLE: Large urban counties with fastest-growing age 0-4 populations since 2020 ##
...
# GRAPH: Change in age 0-4 population, by county type
